...
cast_data_plus <- 
  left_join(cast_data, 
            movies %>% select(id, title, budget, revenue, vote_count, vote_average),
            by=c("movie_id" = "id"))
```

```{r}
cast_data_plus$gender[cast_data_plus$gender==0] <- "Unspecified"
cast_data_plus$gender[cast_data_plus$gender==1] <- "Female"
cast_data_plus$gender[cast_data_plus$gender==2] <- "Male"
cast_data_plus$gender <- factor(cast_data_plus$gender)
```


```{r}
ggplot(cast_data_plus, aes(x =1, fill = gender)) + 
  geom_bar(width = 1, position="dodge") +
  xlab("") +
  ylab("Number of Roles") +
  labs(title="Gender of Players in Roles") +
...
```{r}
genredf_2$flag<-as.vector(unlist(tapply(genredf_2$title,FUN=function(x){1:length(genredf_2)})));
...
```{r}
genredf_2<-subset(genredf, select=-c(id))
...
productionCompanies = movies %>% filter(nchar(production_companies) > 2) %>% mutate(js = lapply(production_companies,fromJSON)) %>% unnest(js) %>% select(id,budget,revenue,production_companies = name)

productionCountries = movies %>% filter(nchar(production_countries) > 2) %>% mutate(js = lapply(production_countries,fromJSON)) %>% unnest(js) %>% select(production_countries = name)


SpokenLang = movies %>% filter(nchar(spoken_languages) > 2) %>% mutate(js = lapply(spoken_languages,fromJSON)) %>% unnest(js) %>% select(iso_639_1,spoken_languages = name)
```

```{r}
movies_2 <- subset(movies, select = -c(genres, keywords, production_companies, production_countries,spoken_languages))
glimpse(movies_2)
```
